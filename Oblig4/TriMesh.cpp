...
namespace GfxUtil {
  ...
  void TriMesh::calcBBox() {
	...
	void TriMesh::buildConnectivity() {
	  ...
	  // unskip
	  std::vector<SortElement> sortedElements;
	  
	  for (size_t i = 0; i < m_halfedges.size(); i++) {
	    sortedElements.push_back(SortElement(m_halfedges[i]));
	  }
	  ...
	  for (i = 0; i < sortedElements.size(); i++) {
	    for (j = i; j < sortedElements.size()-1; j++) {
	      if (sortedElements[j].m_a != sortedElements[j+1].m_a ||
		  sortedElements[j].m_b != sortedElements[j+1].m_b)
		break;
	    }
